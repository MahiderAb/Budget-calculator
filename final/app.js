  ...
  return {
    ...
    addListItem: function (obj, type) {
      ...
      if (type === "inc") {
        ...
        html =
          '<div class="item clearfix p-3 border-b hover:bg-gray-100 transition-all" id="inc-%id%"> <div class="item__description">%description%</div><div class="right clearfix"><div class="item__value">%value%</div><div class="item__delete"><button class="item__delete--btn"><i class="ion-ios-close-outline"></i></button></div></div></div>';
      } else if (type === "exp") {
        element = DOMstrings.expensesContainer;

        html =
          '<div class="item clearfix p-3 border-b hover:bg-gray-100 transition-all" id="exp-%id%"><div class="item__description">%description%</div><div class="right clearfix"><div class="item__value">%value%</div><div class="item__percentage">21%</div><div class="item__delete"><button class="item__delete--btn"><i class="ion-ios-close-outline"></i></button></div></div></div>';
      }
    ...
    updateListItem: function (type, id, field, value) {
      ...
      if (el) {
        if (field === "description") {
          ...
          el.querySelector(".item__value").textContent = formatNumber(
            value,
            type,
          );
        }
...
var controller = (function (budgetCtrl, UICtrl) {
  var setupEventListeners = function () {
    ...
    document
      .querySelector(DOM.container)
      .addEventListener("dblclick", ctrlEditItem);

    // toggle unpaid on expense description click (and update model)
    document
      .querySelector(DOM.expensesContainer)
    ...
    // clear all data
    document
      .querySelector(DOM.clearButton)
      .addEventListener("click", ctrlClearAll);
    // filter unpaid toggle
    document
      .querySelector(DOM.filterCheckbox)
      .addEventListener("change", ctrlFilter);
  };
  ...
  var ctrlClearAll = function () {
    if (confirm("Are you sure you want to clear all entries?")) {
      ...
      var checkbox = document.querySelector(
        UICtrl.getDOMstrings().filterCheckbox,
      );
      if (checkbox) {
  ...
  function ctrlFilter() {
    var showOnly = document.querySelector(
      UICtrl.getDOMstrings().filterCheckbox,
    ).checked;
    var items = document.querySelectorAll(
      UICtrl.getDOMstrings().expensesContainer + " .item",
    );
